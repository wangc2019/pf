...
import quanji from "../../img/homepage/quanji.png";
...
class HomePage extends Component {
  ...
  async get_data(){
    ...
    arr3.push(arr1[0]);
    arr3.push(arr2[0]);
    // this.setState({arr:arr.slice(0, 6) });
    this.setState({arr:arr3 });
  }
        ...
        <div className="guess-list">
          <div className="title"><i></i><span>推荐预测</span></div>
          {
            this.state.arr.map((i,k)=> {
              return(
                i.status=='opening'&&this.CompareDate(this.dateAddDays(i.option.stop,1),mydate.toLocaleDateString()+" "+mydate.getHours()+":"+mydate.getMinutes()+":"+mydate.getSeconds()) &&i.room_type!=0?
                  <div className="guess-content" key={i.id} data-id={i.id} onClick={this.guess.bind(this)}>
                  <div>
                    <p><span>· {i.room_type==1?'奖池型预测 ':'固定赔率 '}</span> <span> 截止时间 {this.dateAddDays(i.option.stop,1)}</span></p>
                    <p>
                      {/*<img src={star} alt=""/>*/}
                    </p>
                  </div>

    ...
    render(){
      ...
      return(
            <div className="currencyDetail">
                <div className="header"><img src={back} onClick={this.goBack.bind(this)} alt=""/>PFC</div>
                    ...
                    <ul>
                      {
                        this.state.history?this.state.history.map(function(item,i) {
                            return(
                              item.op[0]==0&&item.op[1].amount.asset_id=='1.3.2'?
                                <li key={i}>
                                <div>
                                ...
                                <div>
                                  {/*{item.op[0]==50?<p style={{color:'#F13535'}}>-{item.op[1].amount/10000}</p>:null}*/}
                                  {item.op[0]==0&&item.op[1].from==that.getCookie("id")?<p style={{color:'#F13535'}}>-{item.op[1].amount.amount/100000+item.op[1].fee.amount/100000}</p>:null}
                                  ...
                                  {item.op[0]==0&&item.op[1].to==that.getCookie("id")?<p style={{color:"#999",fontSize:"10px"}}>手续费: 0</p>:<p style={{color:"#999",fontSize:"10px"}}>手续费:{item.op[1].fee.amount/100000}</p>}
                                </div>
                              </li>:null
                            )
                        }):null

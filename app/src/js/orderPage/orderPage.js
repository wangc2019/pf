...
class OrderPage extends Component {
  ...
  dateAddDays(dataStr,dayCount) {
    ...
    async componentWillMount() {
      ...
      //通过选项找到该选项当前房间标题
      await dingdan2.map(v=>{
        if(v.operations[0][0]==50){
          info1.push(v);
        }
      });
      info = await  Promise.all(info1.map((v,i)=>{
                ...
                <div className="order-statistics">
                    <div className="order-title">
                        <div className="money-type" onClick={this.onToggleCurrency.bind(this)}>
                            <span className="cur">
                                <img src={wallet} alt=""/><span>PFC</span>
                            </span>
                            {/*<span>*/}
                              ...
                              return(
                                this.state.info[k].status=='opening'?<div key={k}>
                                  <div className="order-content" onClick={this.guess.bind(this)} data-id={v.operations[0][1].room}>
                                    ...
                                    <p>{this.state.info[k].running_option.selection_description[v.operations[0][1].input[0]]}&nbsp;{v.operations[0][1].amount/100000} {this.state.info[k].room_type==0?"份 "+this.state.fanjiang[k].paid/10000+" PFC":"PFC"}{this.state.info[k].room_type==2?<span style={{fontSize:"12px",lineHeight:'42px'}}>(×{this.state.fanjiang[k].amount/this.state.fanjiang[k].paid})</span>:null}</p>
                            ...
                            {this.state.status?this.state.order.map((v,k)=>{
                              return(
                                this.state.info[k].status=='inputing'?<div key={k}>
                                  <div className="order-content" onClick={this.guess.bind(this)} data-id={v.operations[0][1].room}>
                                    <p>{this.state.info[k].description}</p>
                                    <p>{this.state.info[k].running_option.selection_description[v.operations[0][1].input[0]]}&nbsp;{v.operations[0][1].amount/10000} {this.state.info[k].room_type==0?"份":"PFC"}</p>
                                    <p>参与时间: {this.dateAddDays(this.state.fanjiang[k].when,1)}</p>
                                  </div>
                                </div>:null
                              )
                              ...
                              return(
                                this.state.info[k].status=="finished"?<div key={k} data-key={k}>
                                  <div className="order-content" onClick={this.jump.bind(this)} data-id={this.state.order[k].operations[0][1].room}>
                                    <p>{this.state.info[k].description}</p>
                                    <p>{this.state.info[k].running_option.selection_description[this.state.order[k].operations[0][1].input[0]]}&nbsp;{this.state.order[k].operations[0][1].amount/100000} {this.state.info[k].room_type==0?"份":"PFC"}
                                      {this.state.info[k].room_type==2?<span style={{fontSize:"12px",lineHeight:'42px'}}>(×{this.state.fanjiang[k].amount/this.state.fanjiang[k].paid})</span>:null}
                                      {arr[k].reward==0?<span style={{float:"right",color:'#F13535'}}>未中奖</span>:<span style={{float:"right",color:'#34AD56'}}>+ {arr[k].reward/100000} PFC</span>}</p>
                                    <p>参与时间: {this.dateAddDays(this.state.fanjiang[k].when,1)}</p>
                                  </div>
                                </div>:null
                              )
